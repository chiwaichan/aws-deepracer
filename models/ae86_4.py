...
PROGRESS_WEIGHT = 1
IS_ALL_WHEELS_ON_TRACK_WEIGHT = 5
IS_NOT_ALL_WHEELS_ON_TRACK_WEIGHT = 50
...
STRAIGHT_SPEED_5_PERCENT_DEVIATION_ANGLE_WEIGHT = 5.0
STEERING_ANGLE_TRACK_DIRECTION_HEADING_WEIGHT = 15.0

RELATIVELY_STRAIGHT_STEERING_ON_A_STRAIGHT_WEIGHT = 30.0
...
def reward_function(params):
    ...
    is_not_all_wheels_on_track = not all_wheels_on_track
    ...
    track_direction_current_short = get_track_direction(waypoints, waypoints_len, prev_point, 1 * WAYPOINTS_PER_AHEAD_INTERVAL)
    ...
    track_direction_short_from_current_waypoints = get_angle_diff(track_direction_current_short, track_direction)   
    track_direction_current_short_heading_diff = get_angle_diff(track_direction_current_short, heading)   
    track_direction_current_short_heading_diff_abs = abs(track_direction_current_short_heading_diff)

    is_left_turn_coming_up_short = track_direction_short_from_current_waypoints > 0.0
    is_right_turn_coming_up_short = track_direction_short_from_current_waypoints < 0.0

    is_heading_on_left_of_track_direction = track_direction_heading_diff < 0.0
    is_heading_on_right_of_track_direction = track_direction_heading_diff > 0.0

    is_heading_current_short_on_left_of_track_direction = track_direction_current_short_heading_diff < 0.0
    is_heading_current_short_on_right_of_track_direction = track_direction_current_short_heading_diff > 0.0
    
    track_direction_ahead_short_current_track_direction_diff = get_angle_diff(track_direction_ahead_short, track_direction)
    ...
    sub_rewards.append(progress * PROGRESS_WEIGHT)
    ...
    if is_heading_on_left_of_track_direction:
        if track_direction_heading_diff < -90:
            sub_reward_track_direction_heading_diff = -1.2
            
            if percentage_to_edge > 95:
                sub_reward_track_direction_heading_to_edge = -1.2
            elif track_direction_heading_diff > 90:
            ...
            sub_reward_track_direction_heading_diff = -0.2


    elif is_heading_on_right_of_track_direction:
        if track_direction_heading_diff > 90:
            sub_reward_track_direction_heading_diff = -1.2
    ...
    sub_reward_all_wheels_on_track = 0.0

    if all_wheels_on_track:
        sub_reward_all_wheels_on_track = 1.0

        sub_reward_all_wheels_on_track = sub_reward_all_wheels_on_track

    sub_rewards.append(sub_reward_all_wheels_on_track * IS_ALL_WHEELS_ON_TRACK_WEIGHT)



    sub_reward_not_all_wheels_on_track = 0.0
    ...
    sub_reward_track_direction_heading_diff_not_all_wheels_on_track = 0.0
    sub_reward_speed_percentage_not_all_wheels_on_track = 0.0

    if is_offtrack or is_not_all_wheels_on_track:
        if track_direction_heading_diff_abs > 90:
            sub_reward_track_direction_heading_diff_not_all_wheels_on_track = -1.0
        elif track_direction_heading_diff_abs > 70:
            sub_reward_track_direction_heading_diff_not_all_wheels_on_track = -0.8
        elif track_direction_heading_diff_abs > 60:
            sub_reward_track_direction_heading_diff_not_all_wheels_on_track = -0.7
        elif track_direction_heading_diff_abs > 40:
            sub_reward_track_direction_heading_diff_not_all_wheels_on_track = -0.5
        elif track_direction_heading_diff_abs > 20:
            sub_reward_track_direction_heading_diff_not_all_wheels_on_track = -0.3

        sub_reward_speed_percentage_not_all_wheels_on_track = -((max_speed_percentage / 100) + 1)

    sub_rewards.append(sub_reward_track_direction_heading_diff_not_all_wheels_on_track * TRACK_DIRECTION_HEADING_DIFF_NOT_ALL_WHEELS_ON_TRACK_WEIGHT)
    sub_rewards.append(sub_reward_speed_percentage_not_all_wheels_on_track * SPEED_PERCENTAGE_NOT_ALL_WHEELS_ON_TRACK_WEIGHT)
    ...
    straight_speed_5_degree_deviation_angle_count = get_count_of_waypoints_ahead_within_angle(waypoints, waypoints_len, prev_point, 5)

    sub_reward_straight_speed_5_degree_deviation = 0.0

    if straight_speed_5_degree_deviation_angle_count > 5 and track_direction_heading_diff_abs < 10:
        sub_reward_straight_speed_5_degree_deviation = straight_speed_5_degree_deviation_angle_count / 2

        if track_direction_heading_diff_abs == 0.0:
            track_direction_heading_diff_abs = 0.01

        sub_reward_straight_speed_5_degree_deviation = sub_reward_straight_speed_5_degree_deviation / track_direction_heading_diff_abs
        sub_reward_straight_speed_5_degree_deviation = sub_reward_straight_speed_5_degree_deviation * max_speed_percentage / 100.0


    sub_rewards.append(sub_reward_straight_speed_5_degree_deviation * STRAIGHT_SPEED_5_PERCENT_DEVIATION_ANGLE_WEIGHT)
    
    
    # steering_angle_track_direction_heading_diff = get_angle_diff(track_direction_heading_diff, steering_angle)
    # steering_angle_track_direction_heading_diff_abs = abs(steering_angle_track_direction_heading_diff)

    # is_heading_on_left_of_track_direction


    # is_under_heading
    # is_under_steering
    # track_direction_ahead_short
    # reward for aiming towards track direct. Factors in deviation of heading and steering.

    # track_direction_from_current_waypoints
    sub_reward_track_direction_heading_steering_angle = 0.0

    track_direction_steering_state = None
    track_direction_steering_current_short_state = None

    if is_left_turn_coming_up_short:
        
        if track_direction_heading_diff > 90:
            if steering_angle < -25.0:
                sub_reward_track_direction_heading_steering_angle = -2.0
            elif steering_angle < -20.0:
                sub_reward_track_direction_heading_steering_angle = -1.8
            elif steering_angle < -15.0:
                sub_reward_track_direction_heading_steering_angle = -1.4
            elif steering_angle < -5.0:
                sub_reward_track_direction_heading_steering_angle = -1.1
            elif steering_angle < 0.5:
                sub_reward_track_direction_heading_steering_angle = -1.0
            elif steering_angle > 25.0:
                sub_reward_track_direction_heading_steering_angle = 0.7
                ...
                sub_reward_track_direction_heading_steering_angle = 0.1
                
        elif track_direction_heading_diff > 60:
            if steering_angle < -25.0:
                sub_reward_track_direction_heading_steering_angle = -1.7
            elif steering_angle < -20.0:
                sub_reward_track_direction_heading_steering_angle = -1.5
            elif steering_angle < -15.0:
                sub_reward_track_direction_heading_steering_angle = -1.1
            elif steering_angle < -5.0:
                sub_reward_track_direction_heading_steering_angle = -0.8
        ...
        elif track_direction_heading_diff > 30:
            if steering_angle < -25.0:
                sub_reward_track_direction_heading_steering_angle = -1.4
            elif steering_angle < -20.0:
                sub_reward_track_direction_heading_steering_angle = -1.2
                ...
                sub_reward_track_direction_heading_steering_angle = 0.05
        ...
        elif track_direction_heading_diff > 0:
            if steering_angle < -25.0:
                sub_reward_track_direction_heading_steering_angle = -1.0
            elif steering_angle < -20.0:
                sub_reward_track_direction_heading_steering_angle = -0.8
                ...
                sub_reward_track_direction_heading_steering_angle = -0.01
            elif steering_angle > 25.0:
                sub_reward_track_direction_heading_steering_angle = 0.1
            elif steering_angle > 10.0:
                sub_reward_track_direction_heading_steering_angle = 0.6
            ...
            # tracking even better in the short distance
            if track_direction_current_short_heading_diff_abs < 4.0:
                track_direction_steering_current_short_state = None

        # understeering while turning left
        elif is_heading_on_right_of_track_direction:
            under_steering = True


    sub_rewards.append(sub_reward_track_direction_heading_steering_angle * STEERING_ANGLE_TRACK_DIRECTION_HEADING_WEIGHT)

    # penalise when heading along track direction and steering is heaps


    # reward for steering less than 8 degrees when heading within 10 in the right track direction on a straight

    sub_reward_relatively_straight_steering_on_a_straight = 0.0

    if straight_speed_5_degree_deviation_angle_count > 5:
        if track_direction_heading_diff_abs < 10.0:
            if steering_angle_abs < 8.0:
                sub_reward_relatively_straight_steering_on_a_straight = straight_speed_5_degree_deviation_angle_count / 2
                
                sub_reward_relatively_straight_steering_on_a_straight = sub_reward_relatively_straight_steering_on_a_straight * max_speed_percentage / 100.0

                if track_direction_heading_diff_abs < 5:
                    sub_reward_relatively_straight_steering_on_a_straight *= 1.2

                if steering_angle_abs < 4:
                    sub_reward_relatively_straight_steering_on_a_straight *= 1.2

    sub_rewards.append(sub_reward_relatively_straight_steering_on_a_straight * RELATIVELY_STRAIGHT_STEERING_ON_A_STRAIGHT_WEIGHT)
    ...
    vs = [
            ...
            is_offtrack,
            sub_reward_is_offtrack,
            
            all_wheels_on_track,
            sub_reward_all_wheels_on_track,
            is_not_all_wheels_on_track,
            sub_reward_not_all_wheels_on_track,
            sub_reward_speed_percentage_not_all_wheels_on_track,
            sub_reward_track_direction_heading_diff_not_all_wheels_on_track,
            ...
            speed,
            steering_angle,
            sub_reward_relatively_straight_steering_on_a_straight,
            ...
            track_direction_ahead_short,
            track_direction_ahead_short_heading_diff,
            track_direction_ahead_short_current_track_direction_diff,
            ...
            IS_NOT_ALL_WHEELS_ON_TRACK_WEIGHT,
            IS_REVERSED_WEIGHT,
            SPEED_PERCENTAGE_NOT_ALL_WHEELS_ON_TRACK_WEIGHT,
            TRACK_DIRECTION_HEADING_DIFF_NOT_ALL_WHEELS_ON_TRACK_WEIGHT,
            STRAIGHT_SPEED_5_PERCENT_DEVIATION_ANGLE_WEIGHT,
            STEERING_ANGLE_TRACK_DIRECTION_HEADING_WEIGHT,
            DIRECTION_DIFF_WEIGHT,
            LONG_DISTANCE_AHEAD_HEADING_WEIGHT,
            STEERING_ANGLE_DIRECTION_HEADING_DIFF_WEIGHT,
            RELATIVELY_STRAIGHT_STEERING_ON_A_STRAIGHT_WEIGHT,

            straight_speed_5_degree_deviation_angle_count
...
def get_angle_diff(angle_b, angle_a, normalise=True):
    diff = angle_b - angle_a

    return get_normalised_angle(diff) if normalise else diff
...
def get_percentage_to_edge(distance_from_center, track_width):
    half_track_width = track_width / 2.0
